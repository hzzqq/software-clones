...
```bash
# 前端
cd tvtime/client && npm install && npm run dev

# 后端
cd tvtime/server && npm install && npm run dev
```

后端默认从 `.env`（见 `server/.env.example`）读取 `PORT` / `CORS_ORIGIN` / `DB_PATH`，
首次启动自动建表（读 `src/schema.sql`），SQLite 落在 `tvtime/server/data/app.db`。
