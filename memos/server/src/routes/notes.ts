...
notesRouter.post(
  ...
  asyncHandler((req: Request, res: Response): void => {
    ...
    const note = createNote({ content: content.trim(), visibility: vis, tags: parseTags(content) });
...
notesRouter.post(
  '/notes/:id/archive',
  asyncHandler((req: Request, res: Response): void => {
    const note = setArchived(Number(req.params.id), true);
    if (!note) throw new HttpError(404, 40400, 'note not found');
    res.json({ code: 0, message: 'ok', data: note });
  }),
);
...
notesRouter.delete(
  '/notes/:id/archive',
  asyncHandler((req: Request, res: Response): void => {
    const note = setArchived(Number(req.params.id), false);
    if (!note) throw new HttpError(404, 40400, 'note not found');
    res.json({ code: 0, message: 'ok', data: note });
  }),
);
...
notesRouter.post(
  '/notes/:id/pin',
  asyncHandler((req: Request, res: Response): void => {
    const note = setPinned(Number(req.params.id), true);
    if (!note) throw new HttpError(404, 40400, 'note not found');
    res.json({ code: 0, message: 'ok', data: note });
  }),
);
...
notesRouter.delete(
  '/notes/:id/pin',
  asyncHandler((req: Request, res: Response): void => {
    const note = setPinned(Number(req.params.id), false);
    if (!note) throw new HttpError(404, 40400, 'note not found');
    res.json({ code: 0, message: 'ok', data: note });
  }),
);
