...
```bash
# 前端
cd lofi/client && npm install && npm run dev

# 后端
cd lofi/server && npm install && npm run dev
```

后端默认从 `.env`（见 `server/.env.example`）读取 `PORT` / `CORS_ORIGIN` / `DB_PATH`，
首次启动自动建表（读 `src/schema.sql`），SQLite 落在 `lofi/server/data/app.db`。
