...
function main() {
  ...
  for (const app of apps) {
    ...
    const checks = {
      ...
      timeoutGuard: apiClientHasTimeout(ROOT, app).length === 0,
      serverSecHeaders: serverSecurityHeaders(ROOT, app).length === 0,
    ...
    if (!checks.timeoutGuard) errors.push(`[${app}] api/client.ts 未对请求设置超时（应基于 AbortController 在超时后中断 fetch，并对中断给出错误码 50002）`);
    if (!checks.serverSecHeaders) errors.push(`[${app}] server 未设置基础安全响应头（server/src/middleware/securityHeaders.ts 缺失或未在 app.ts 注册）`);
  ...
  // 脚手架模板的 API 客户端同样必须设置请求超时（新克隆的 App 才自带该兜底）
  for (const pr of apiClientHasTimeout(ROOT, 'shared/frontend-template', 'src/api/client.ts')) {
    errors.push(`[shared-template] ${pr}`);
  }
  ...
  for (const pr of hasGitHook(ROOT)) {
    errors.push(`[git-hook] ${pr}`);
  }

  // 安全纵深防御：扫描 App 源码，禁止提交硬编码密钥/凭据字面量（避免凭证泄露）
  for (const s of noHardcodedSecrets(ROOT)) {
    errors.push(`[secrets] 疑似硬编码密钥/凭据：${s}`);
  }
  ...
  const RULES = [
    ...
    'catalog-fresh', 'license', 'gitignore-artifacts', 'readme-verify', 'contributing', 'ci-runs-unit-tests', 'git-hook', 'nvmrc', 'no-hardcoded-secrets', 'api-network-guard', 'api-timeout-guard', 'server-security-headers', 'global-error-handler',
  ];
