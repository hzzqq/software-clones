...
开发者工具箱：30+ 常用工具（编解码、格式化、转换等），收藏与历史可选后端。
...
```bash
# 前端
cd it-tools/client && npm install && npm run dev

# 后端
cd it-tools/server && npm install && npm run dev
```

后端默认从 `.env`（见 `server/.env.example`）读取 `PORT` / `CORS_ORIGIN` / `DB_PATH`，
首次启动自动建表（读 `src/schema.sql`），SQLite 落在 `it-tools/server/data/app.db`。
