...
信息聚合仪表盘：RSS / 天气 / 书签 / 状态 / 时钟等小组件可配置布局。
...
```bash
# 前端
cd glance/client && npm install && npm run dev

# 后端
cd glance/server && npm install && npm run dev
```

后端默认从 `.env`（见 `server/.env.example`）读取 `PORT` / `CORS_ORIGIN` / `DB_PATH`，
首次启动自动建表（读 `src/schema.sql`），SQLite 落在 `glance/server/data/app.db`。
